...
from cova import utils, REF

## Functions ##
def extract_metadata_gisaid_table(fin,fout):
	df = pandas.read_csv(fin, sep='\t', usecols=['gisaid_epi_isl', 'date', 'division'])
	df['gisaid_epi_isl'] = df.gisaid_epi_isl.str.replace('EPI_ISL_','')
	out = df
	out.columns = ['accession','date','location']
