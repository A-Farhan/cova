...
#define RECURSIVETOP 1

#define ENABLEMPIDEBUG 0
//the rough expectation number of chunks which will be assigned to each process
#define GRANULARITY 10


static char lockfile[1000];
static int lockthisjobonly;
#if enableatomic
static ATOMICINT *lockaddr;
#else
#if SEMAPHORE
static sem_t *lockaddr;
#else
static pthread_mutex_t *lockaddr;
#endif
#endif
static int lockfd;
int createshm;
...
static int compfuncpairpt( const void *p, const void *q )
{
	return( (*(Pairnum **)q)->npairs - (*(Pairnum **)p)->npairs );
}
...
#if 0
static int *topolordery( int *order, int ***topol, Treedep *dep, int pos, int nchild )
{
	int child0, child1;
	...
	if( nchild == 0 || nchild == 2 )
	{
		...
		{
			order = topolordery( order, topol, dep, child0, 2 );
		}
	}
	...
	if( nchild == 1 || nchild == 2 )
	{
		...
		{
			order = topolordery( order, topol, dep, child1, 2 );
		}
	}
...
void topolorder( int *order, int *posinorder, int ***topol, Treedep *dep, int pos, int nchild )
{
	int *str;
	int child0, child1;
	child0 = dep[pos].child0;
...
//			intcat( order, str );
		...
		{
			topolorder( order, posinorder, topol, dep, child0, 2 );
		}
	}
...
//			intcat( order, str );
		...
		{
			topolorder( order, posinorder, topol, dep, child1, 2 );
		}
	}
//	return( posinorder );
}
...
static int *topolorder_lessargs( int *order, TopDep *tdp, int pos )
{
	int child;

	if( (child=(tdp->dep)[pos].child0) == -1 )
	{
		*order++ = (tdp->topol)[pos][0][0];
		*order = -1;
	}
	else
	{
		order = topolorder_lessargs( order, tdp, child );
	}

	if( (child=(tdp->dep)[pos].child1) == -1 )
	{
		*order++ = (tdp->topol)[pos][1][0];
		*order = -1;
	}
	else
	{
...
int *topolorderz( int *order, int ***topol, Treedep *dep, int pos, int nchild )
{
	...
	if( nchild == 0 || nchild == 2 )
	{
		if( (child=(td.dep)[pos].child0) == -1 )
		{
			*order++ = (td.topol)[pos][0][0];
			*order = -1;
		}
		else
		{
	...
	if( nchild == 1 || nchild == 2 )
	{
		if( (child=(td.dep)[pos].child1) == -1 )
		{
			*order++ = (td.topol)[pos][1][0];
			*order = -1;
		}
		else
		{
...
static void reformattree( Treept *root, Treept *ori, int n, int ***topol, double **len, Treedep *dep, char **name, int treeout, int my_rank ) // my_rank tsuika, 2017/Mar/6
{
	int i, pos;
	char *newick;
	int *lastappear;
...
//	reporterr( "njob=%d, treept[%d].parent,child0,child1 - self = %p,%p,%p - %p\n", n, i, treept[i].parent,treept[i].child0,treept[i].child1,treept+i );
	if( treeout && my_rank == 0 ) // my_rank tsuika, 2017/Mar/6
//	if( treeout )
	{
//		reporterr( "treeout!\n" );
		FILE *fp;
...
//			sprintf( tree[i], "%d_l=%d_%.20s", i+1, nlen[i], nametmp+1 );
		...
		fp = fopen( "infile.tree", "w" ); // my_rank==0 nomi
		fprintf( fp, "%s;\n", newick );
		fclose( fp );
...
#if 0
static int myfwrite( void *ptr, size_t unit, int num, FILE *fp )
{
	int lim=10;
	while( lim-- )
	{
		if( fwrite( ptr, unit, num, fp ) == num ) return( num );
//		system( "hostname 1>&2" );
//		system( "df 1>&2" );
//		system( "lfs df 1>&2" );
//		system( "cat /proc/fs/lustre/lov/work2-clilov-*/target_obd 1>&2" );
		reporterr( "error in fwrite(), retrying, lim=%d\n", lim );
		sleep( 1 ); 
	}
	reporterr( "error in myfwrite()" );
	exit( 1 );
}
#endif

static int writehat3node_noaddress( int n, int i, int j, int ii, int jj, FILE **fpp, int tmpint, LocalHom *lh, int *uselh )
{
...
//				reporterr( "reg1:%d-%d, reg2:%d-%d, len=%d, score=%f\n", tmpptr->start1, tmpptr->start1+len, tmpptr->start2, tmpptr->start2+len, len, opt );
			...
			c = '\n';
			if( fwrite( &c, sizeof( char ), 1, *fpp ) != 1 )
			{
				reporterr( "write error, n=%d\n", n );
				return( -1 );
			}
...
#if HAT3SORTED
		return( size * 3 * sizeof( int ) + sizeof( char ) + sizeof( int ) + sizeof( double ) );
#else
		return( size * 3 * sizeof( int ) + sizeof( char ) + 3 * sizeof( int ) + sizeof( double ) );
#endif
	}
}
...
typedef struct _calcnearestthread_arg_mpi
{
	char **bseq;
	int thread_no;
	int *posshared;
	...
	// int nlim;
	double *selfscore;
	double *mindists_partial;
	int *neighbors_partial;
	int start;
	int end;
	pthread_mutex_t *mutex;
} calcnearestthread_arg_t_mpi;

static void *calcnearestthread_mpi( void *arg )
{
	calcnearestthread_arg_t_mpi *targ = (calcnearestthread_arg_t_mpi *)arg;
	char **bseq = targ->bseq;
//	int thread_no = targ->thread_no;
	int *posshared = targ->posshared;
	int alloclen = targ->alloclen;
	// int nlim = targ->nlim;
	double *selfscore = targ->selfscore;
	double *mindists_partial = targ->mindists_partial;
	int *neighbors_partial = targ->neighbors_partial;
...
//		for( i=pos+1; i<njob; i++ )
		...
		mindists_partial[pos-start] = mindist;
		neighbors_partial[pos-start] = neighbor;
	}
}
...
static void flushlockinit( int my_rank )
{
	int namelen;
	int ntry;
	if( my_rank == 0 )
	{
		...
		lockthisjobonly = 0;
		struct stat st;
		system( "lfs getstripe . > _lfscheck 2>/dev/null" );
		stat( "_lfscheck", &st );
		if( 1 || st.st_size == 0 )
		{
			lockthisjobonly = 1;
			reporterr( "Use shm lock.  This job only.\n" );
			sprintf( lockfile, "/mafftflush-%s-%d", getenv( "HOSTNAME" ), getpid() );
//			reporterr( "Use filesystem's lock\n" );
//			lockfile[0]=0;
//			namelen = 0;
			namelen = strlen( lockfile ) + 1;
		}
		else
		{
//			lockthisjobonly = 1;
//			reporterr( "Use intra-job lock.\n" );
//			sprintf( lockfile, "/tmp/mafftflush-%s-%d", getenv( "HOSTNAME" ), getpid() );
			lockthisjobonly = 0;
			reporterr( "Use shm lock.  Lock other jobs, too.\n" );
			sprintf( lockfile, "/mafftflush-%s", getenv( "USER" ) );
			namelen = strlen( lockfile ) + 1;
		}
		reporterr( "rank=%d, lockfile=%s\n", my_rank, lockfile );
#else
		lockthisjobonly = 1;
		sprintf( lockfile, "/mafftflush-%s-%d", getenv( "HOSTNAME" ), getpid() );
#ifdef enableatomic
		reporterr( "Use shm, semaphore, use stdatomic.h, key=%s\n", lockfile );
#else
		reporterr( "Use shm, pthread's shared mutex or semaphore, key=%s\n", lockfile );
#endif
		namelen = strlen( lockfile ) + 1;
#endif
	}
	...
	MPI_Bcast(lockfile, namelen, MPI_CHAR, 0, MPI_COMM_WORLD);
	MPI_Bcast(&lockthisjobonly, 1, MPI_INT, 0, MPI_COMM_WORLD);


#ifdef enableatomic
	lockfd = shm_open( lockfile, O_RDWR|O_CREAT|O_EXCL, 0600 );
	if( lockfd == -1 )
	{
		lockfd = shm_open( lockfile, O_RDWR, 0600 );
//		reporterr( "Trying to open existing shm %s on %s, without re-ftruncate, fixed mkdircommand allocation and handling fflush failure.  Fixed a bug in flushlock\n", lockfile, getenv("HOSTNAME") );
		if( lockfd == -1 )
		{
			reporterr( "Failed to open %s on %s\n", lockfile, getenv("HOSTNAME") );
			exit( 1 );
		}
	}
	else reporterr( "Trying to create and initialize shm %s on %s\n", lockfile, getenv("HOSTNAME") );

//		reporterr( "lockfd=%d, (rank=%d, host=%s)\n", lockfd, my_rank, getenv("HOSTNAME") );
	
	if( ftruncate( lockfd, sizeof(ATOMICINT) ) != 0 ) // naito tomaru..
	{
		perror("ftruncate");
		exit( 1 );
	}

	lockaddr = (ATOMICINT *)mmap( NULL, sizeof(ATOMICINT), PROT_READ|PROT_WRITE, MAP_SHARED, lockfd, 0 );
	if( lockaddr == MAP_FAILED )
	{
		reporterr( "error in mmap on %s\n", getenv("HOSTNAME") );
		exit( 1 );
	}
	atomic_store( lockaddr, 0 ); // nenn no tame

#else
	lockfd = shm_open( lockfile, O_RDWR|O_CREAT|O_EXCL, 0600 );
	if( lockfd == -1 )
	{
//		reporterr( "Use existing lockfile in %s. Initialize it in each process\n", getenv("HOSTNAME") );
		lockfd = shm_open( lockfile, O_RDWR, 0600 );
		if( lockfd == -1 )
		{
			reporterr( "Failed to open %s on %s\n", lockfile, getenv("HOSTNAME") );
	...
	{
#if SEMAPHORE
		reporterr( "trying to create shm %s for semaphore, NSTREAM=%d\n", lockfile, NSTREAM );
		if( ftruncate(lockfd, sizeof(sem_t)) != 0 )
#else
		if( ftruncate(lockfd, sizeof(pthread_mutex_t)) != 0 )
#endif
		{
			perror( "ftruncate" );
			exit( 1 );
		...
		createshm = 1;
		reporterr( "Created and truncated lockfile exclusively in %s.\n", getenv("HOSTNAME") );
	}

	for( ntry=0; ntry<10; ntry++ ) // mada ftruncate dekite naikamo shirenainode
	{
//		reporterr( "trying to mmap in %s\n", getenv("HOSTNAME") );
#if SEMAPHORE
		reporterr( "trying to mmap in %s for semaphore, NSTREAM=%d\n", getenv("HOSTNAME"), NSTREAM );
		lockaddr = (sem_t *)mmap( NULL, sizeof(sem_t), PROT_READ|PROT_WRITE, MAP_SHARED, lockfd, 0 );
#else
		lockaddr = (pthread_mutex_t *)mmap( NULL, sizeof(pthread_mutex_t), PROT_READ|PROT_WRITE, MAP_SHARED, lockfd, 0 );
#endif
		if( lockaddr == MAP_FAILED ) 
		{
			perror( "mmap" );
...
//		reporterr( "succeeded to mmap, size= %d\n", sizeof(pthread_mutex_t) );
		break;
	}
	if( ntry == 10 )
	{
		reporterr( "Cannot mmap %s in %s\n", lockfile, getenv("HOSTNAME") );
		exit( 1 );
	}

	if( createshm ) // hokademo shokika sureba kantan ni naru
	{
#if SEMAPHORE
		if( sem_init( lockaddr, 1, NSTREAM ) )
		{
			perror("sem_init");
//			exit( 1 ); // POSIX.1-2001 does not always return zero.
		}
#else
		...
		if(pthread_mutex_init(lockaddr, &attr)) 
		{
			perror("pthread_mutex_init");
//			reporterr( "Probably because of duplicate initialization of shared mutex.  Can be ignored.\n" );
			exit( 1 );
		}
#endif
	}
	...
	MPI_Barrier(MPI_COMM_WORLD); // shokika no mae ni tsukawareruno wo fusegu.
//	reporterr( "Successfully mapped SHM %s on %s. val=%d\n", lockfile, getenv( "HOSTNAME" ), *lockaddr );
}

static void flushlockremove()
{
	if( lockaddr == NULL ) return; // nai

#ifdef enableatomic
	if( munmap( lockaddr, sizeof(ATOMICINT) ) )
	{
		reporterr( "Failed to unmap SHM\n" );
		exit( 1 );
	}
#else
#if SEMAPHORE
	if( createshm == 1 )
	{
		reporterr( "sem_destroy in %s\n", getenv("HOSTNAME") );
		if( sem_destroy(lockaddr) )
		{
			perror("sem_destroy");
			//exit( 1 ); // tajuu ni destroy.
		}
	}
	if( munmap( lockaddr, sizeof(sem_t) ) )
	{
		reporterr( "Failed to unmap SHM\n" );
		exit( 1 );
	}
#else
//	if( pthread_mutex_destroy(lockaddr) )
//	{
//		perror("pthread_mutex_destroy");
//		exit( 1 );
//	}
	reporterr( "Did not destroy mutex.\n" );
	if( munmap( lockaddr, sizeof(pthread_mutex_t) ) )
	{
		reporterr( "Failed to unmap SHM\n" );
		exit( 1 );
	}
	...
	if( lockthisjobonly )
	{
//		if( shm_unlink( lockfile ) ) perror( "shm_unlink" );
		shm_unlink( lockfile ); // No such file -> no problem
		reporterr( "Unmapped, closed and unlinked SHM.\n" );
	}
	else
		reporterr( "Successfully unmapped and closed SHM, but did not unlink.\n" );
}

static void flushlock()
{
	if( lockaddr == NULL ) return;

#ifdef enableatomic
	int interval = 10;
	while( 1 )
	{
		if( atomic_fetch_add( lockaddr, 1 ) < NSTREAM ) // atomic
			return;

		atomic_fetch_sub( lockaddr, 1 ); // kouritsu warui?
...
#ifdef enableatomic
	int tmpint;
//	atomic_fetch_sub( lockaddr, 1 );
	if( (tmpint=atomic_fetch_sub( lockaddr, 1 )) > NSTREAM )
	{
		reporterr( "Strange.  *lockaddr == %d before subtraction (too large), %s\n", tmpint, getenv("HOSTNAME") );
		atomic_store( lockaddr, NSTREAM-1 );
	}
	else if( tmpint < 1 ) 
	{
		reporterr( "Strange.  *lockaddr == %d before subtraction (too small), %s\n", tmpint, getenv("HOSTNAME") );
		atomic_store( lockaddr, 0 );
	}
#else
#if SEMAPHORE
	if( sem_post(lockaddr) )
	{
		perror("sem_post");
		exit( 1 );
	}
...
typedef struct _recalcpairs4thread_arg_mpi
{
//	int thread_no;
	int nseq;
//	int numjob;
	...
	pthread_mutex_t *mutex;
	int my_rank;
} recalcpairs4thread_arg_t_mpi;


static void *recalcpairs4thread_mpi( void *arg )// no TLS
{
	recalcpairs4thread_arg_t_mpi *targ = (recalcpairs4thread_arg_t_mpi *)arg;
	char **bseq = targ->bseq;
	int nseq=targ->nseq;
...
//	int thread_no = targ->thread_no;
	...
	int my_rank = targ->my_rank;

	int writetry, writeres;
...
//		if( *done % 100 == 0 )
		...
		for( writetry=0; writetry<10; writetry++ )
		{
			flushlock();
			localfp = fopen( fn, "wb" );
			if( !localfp )
			{
				char *mkdircommand;
				mkdircommand = calloc( sizeof( char ), 1000 );
				sprintf( mkdircommand, "mkdir -p hat3dir/%d-/", (int)(n/HAT3NODEBLOCK)*HAT3NODEBLOCK );
				system( mkdircommand );
				free( mkdircommand );

				reporterr( "Failed to open %s but will retry (%d).\n", fn, writetry );
			...
			if( setvbuf( localfp, stbuf, _IOFBF, MYBUFSIZE ) )
			{
				reporterr( "Cannot change the buffer size to %d\n", MYBUFSIZE );
				flushunlock();
				exit( 1 );
			}
			flushunlock();
...
//					reporterr( "tmpdist = %f\n", tmpdist );
#if N0LOOPFIRST
					writeres = writehat3node_noaddress( n, m0, m1, j, i, &localfp, 'n', localhomtable, uselh );
#else
					writeres = writehat3node_noaddress( n, m0, m1, i, j, &localfp, 'n', localhomtable, uselh );
#endif

					freelocalhom1( localhomtable );
					if( writeres == -1 )
					{
						reporterr( "Failed to write %s (%d). rank=%d\n", fn, writetry, my_rank );
						reporterr( "sizewritten=%d, MYBUFSIZE=%d, writeres=%d\n", sizewritten, MYBUFSIZE, writeres );
						sleep( 10 );
						goto rewrite;
					}

					sizewritten += writeres;
					if( sizewritten > MYBUFSIZE - 4000 ) // avoid automatic flush
					{
						flushlock();
//						reporterr( "flush at the middle of a file, MYBUFSIZE=%d, sizewritten=%d, writeres=%d, file=%s\n", MYBUFSIZE, sizewritten, writeres, fn );
//						reporterr( "sizewritten=%d, flushing %s\n", sizewritten, fn );
						sizewritten = 0;
						if( fflush( localfp ) )
						{
							reporterr( "Failed to fflush in the middle of a file, rank=%d, hostname=%s\n", my_rank, getenv( "HOSTNAME" ) );
							system( "date" );
							sleep( 10 );
							flushunlock(); // fclose no ato no houga anzen.
							continue;
						}
						flushunlock();
					}
			...
			if( fflush( localfp ) )
			{
				reporterr( "Failed to fflush %s. rank=%d, hostname=%s\n", fn, my_rank, getenv( "HOSTNAME" ) );
				system( "date" );
				sleep( 10 );
				//__fpurge( localfp );
				flushunlock(); // fclose no ato no houga anzen.
...
//			flushunlock(); // fclose no ato no houga anzen.
			...
			if( fclose( localfp ) )
			{
				reporterr( "Failed to close %s (%d). rank=%d\n", fn, writetry, my_rank );
				sleep( 10 );
				flushunlock(); // fclose no ato no houga anzen.
				continue;
			}
			flushunlock(); // koko no houga anzen.

//			if( !writeres )
//			{
//				char com[100];
//				sprintf( com, "ls -ltr %s; hostname", fn );
//				reporterr( "wrote %s successfully, rank=%d\n", fn, my_rank );
//				system( com );
				break;
//			}
...
//
			rewrite:
			reporterr( "Will not close localfp. filenamd=%s, rank=%d\n", fn, my_rank );
			;
		}

		if( writetry == 10 )
		{
			reporterr( "Failed to write %s 10 times in %s.  Give up.\n", fn, getenv( "HOSTNAME" ) );
			system( "hostname" );
			exit( 1 );
		}
...
typedef struct _calcnearest_para_thread_rank0_mpi_arg
{
//	const int njob;
	const unsigned int chunksize;
//	const unsigned int num_of_chunks;
	int *neighbors;
	double *mindists;
	int *chunk_counter_pt;
	pthread_mutex_t *mutex_count;
} calcnearest_para_thread_rank0_mpi_arg_t;

static void *calcnearest_para_thread_rank0_mpi(void *arg){
	calcnearest_para_thread_rank0_mpi_arg_t *targ = (calcnearest_para_thread_rank0_mpi_arg_t *)arg;
//	const int njob = targ->njob;
	const unsigned int chunksize = targ->chunksize;
//	const unsigned int num_of_chunks = targ->num_of_chunks;
	int *neighbors = targ->neighbors;
	double *mindists = targ->mindists;
	int *chunk_counter_pt = targ->chunk_counter_pt;
	pthread_mutex_t *mutex_count = targ->mutex_count;
	...
	enum {TODO,DONE,EXIT};
	MPI_Status status;
	unsigned int mpi_chunk_balance = 0;
//	unsigned short *const chunks_assignment = (unsigned short *)malloc(num_of_chunks*sizeof(unsigned short));

//	const int first_child = nthread>1?0:1;
	const int first_child = 1;

	pthread_mutex_lock( mutex_count );
//	if( nthread > 1 )
//	{
//		fprintf( stderr, "my_rank: %d, chunks_assignment: %d, 0\n", my_rank, *chunk_counter_pt);
//		(*chunk_counter_pt)--;
//	}

	for(i=first_child;i<num_of_processes;i++){
		if(*chunk_counter_pt<0) break;
		MPI_Send(chunk_counter_pt,1,MPI_INT,i,TODO,MPI_COMM_WORLD);
//		chunks_assignment[*chunk_counter_pt]=i;
		fprintf( stderr, "my_rank: %d, chunks_assignment: %d, %d\n", my_rank, *chunk_counter_pt, i);
		(*chunk_counter_pt)--;
		mpi_chunk_balance++;
	}
	...
	while(mpi_chunk_balance){
		int chunk_counter;
		int chunk_index;
		MPI_Recv(&chunk_index,1,MPI_INT,MPI_ANY_SOURCE,DONE,MPI_COMM_WORLD,&status);

		int sender_id = status.MPI_SOURCE;
		unsigned int num_of_jobs;

		MPI_Recv(&num_of_jobs,1,MPI_INT,sender_id,DONE,MPI_COMM_WORLD,&status);
		MPI_Recv(&neighbors[chunk_index*chunksize+1],num_of_jobs,MPI_UNSIGNED,sender_id,DONE,MPI_COMM_WORLD,&status);
		#if ENABLEMPIDEBUG
		for(i=chunk_index*chunksize+1;i<((chunk_index+1)*chunksize+1>njob?njob:(chunk_index+1)*chunksize+1);i++) fprintf( stderr, "%d, ", neighbors[i]);
		fprintf( stderr, "\n");
		#endif
		MPI_Recv(&mindists[chunk_index*chunksize+1],num_of_jobs,MPI_DOUBLE,sender_id,DONE,MPI_COMM_WORLD,&status);
		#if ENABLEMPIDEBUG
		for(i=chunk_index*chunksize+1;i<((chunk_index+1)*chunksize>njob?njob:(chunk_index+1)*chunksize+1);i++) fprintf( stderr, "%lf, ", mindists[i]);
		fprintf( stderr, "\n");
		#endif

		pthread_mutex_lock( mutex_count );
		chunk_counter = (*chunk_counter_pt)--;
		pthread_mutex_unlock( mutex_count );

		if(chunk_counter>=0){
			MPI_Send(&chunk_counter,1,MPI_INT,sender_id,TODO,MPI_COMM_WORLD);
//			chunks_assignment[*chunk_counter_pt]=sender_id;
			fprintf( stderr, "my_rank: %d, chunks_assignment: %d, %d\n", my_rank, chunk_counter, sender_id);
			mpi_chunk_balance++;
		}
		mpi_chunk_balance--;
	}
	...
	for(i=first_child;i<num_of_processes;i++){
		MPI_Send(chunk_counter_pt,1,MPI_INT,i,EXIT,MPI_COMM_WORLD);
	}

//	free(chunks_assignment);
	return(NULL);
}

typedef struct _calcthreads_dp1_arg
{
	const int njob;
	const unsigned int chunksize;
	const int my_rank;
	...
	int *chunk_counter_pt;
	pthread_mutex_t *mutex_count;
} calcthreads_dp1_arg_t;

static void *calcthreads_dp1(void *arg)
{
	calcthreads_dp1_arg_t *sarg = (calcthreads_dp1_arg_t *)arg;
	const int my_rank = sarg->my_rank;

	const int num_thread = my_rank==0?nthread-1:nthread;

	if(num_thread>0){
		const int njob = sarg->njob;
		const unsigned int chunksize = sarg->chunksize;
		char **bseq = sarg->bseq;
		double *selfscore = sarg->selfscore;
		int alloclen = sarg->alloclen;
		int *neighbors = sarg->neighbors;
		double *mindists = sarg->mindists;
		int *chunk_counter_pt = sarg->chunk_counter_pt;
		pthread_mutex_t *mutex_count = sarg->mutex_count;
		enum {TODO,DONE,EXIT};
		MPI_Status status;
		int *neighbors_partial = (int *)malloc(chunksize*sizeof(int));
		double *mindists_partial = (double *)malloc(chunksize*sizeof(double));
		int i;

		calcnearestthread_arg_t_mpi *targ;
		pthread_t *handle;
		pthread_mutex_t mutex;
		int posshared;
		int chunk_index;
		...
		while(1){
			if( my_rank == 0 ){
				pthread_mutex_lock( mutex_count );
				chunk_index = (*chunk_counter_pt)--;
				pthread_mutex_unlock( mutex_count );
				if(chunk_index<0) break;

				fprintf( stderr, "my_rank: %d, chunks_assignment: %d, 0\n", my_rank, chunk_index);

			} else {
				MPI_Recv(&chunk_index,1,MPI_UNSIGNED,0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
				if(status.MPI_TAG==EXIT) break;
			}
	
			int start = chunk_index*chunksize+1;
			int end = (chunk_index+1)*chunksize+1>njob?njob:(chunk_index+1)*chunksize+1;
			int num_of_jobs = end-start;

		#if ENABLEMPIDEBUG
			fprintf( stderr, "my_rank: %d, chunk_index: %d, start: %d, end: %d, nthread: %d\n", my_rank, chunk_index, start, end, nthread);
		#endif

			posshared = end-1;
			targ = calloc( num_thread, sizeof( calcnearestthread_arg_t_mpi ) );
			handle = calloc( num_thread, sizeof( pthread_t ) );
			pthread_mutex_init( &mutex, NULL );
			...
			for( i=0; i<num_thread; i++ )
			{
				...
				targ[i].selfscore = selfscore;
				targ[i].alloclen = alloclen;
				targ[i].neighbors_partial = neighbors_partial;
				targ[i].mindists_partial = mindists_partial;
				targ[i].mutex = &mutex;
				targ[i].start = start;
				targ[i].end = end;
				pthread_create( handle+i, NULL, calcnearestthread_mpi, (void *)(targ+i) );
			}

			for( i=0; i<num_thread; i++ ) pthread_join( handle[i], NULL );
			pthread_mutex_destroy( &mutex );
			free( handle );
			free( targ );
			...
			if( my_rank == 0 )
			{
				intncpy( neighbors+chunk_index*chunksize+1, neighbors_partial, num_of_jobs );
				fltncpy( mindists+chunk_index*chunksize+1, mindists_partial, num_of_jobs );

//				pthread_mutex_lock( mutex_count );
//				if(*chunk_counter_pt>=0){
//					chunk_index = (*chunk_counter_pt)--;
//				}else{
//					rank0status = EXIT; // ???
//				}
//				pthread_mutex_unlock( mutex_count );
//
//				if(rank0status!=EXIT) fprintf( stderr, "my_rank: %d, chunks_assignment: %d, 0\n", my_rank, chunk_index);
			}
			else
			{
				MPI_Send(&chunk_index,1,MPI_INT,0,DONE,MPI_COMM_WORLD);
				MPI_Send(&num_of_jobs,1,MPI_INT,0,DONE,MPI_COMM_WORLD);
				MPI_Send(neighbors_partial,num_of_jobs,MPI_UNSIGNED,0,DONE,MPI_COMM_WORLD);
				MPI_Send(mindists_partial,num_of_jobs,MPI_DOUBLE,0,DONE,MPI_COMM_WORLD);
//				MPI_Recv(&chunk_index,1,MPI_INT,0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
			}
		}

		free(neighbors_partial);
		free(mindists_partial);
	}
	return( NULL );
}

static void calcnearest_para_mpi(int njob, double *selfscore, char **bseq, int alloclen, int *neighbors, double *mindists )
{
#if REPORTCOSTS
	time_t starttime, startclock;
	starttime = time(NULL);
	...
	if(num_of_processes*nthread < 2){
		if(my_rank==0) reporterr( "(number of processes)*(number of threads) must be more than 1\n");
		if(my_rank==0) reporterr( "number of processes: %d, number of threads: %d\n", num_of_processes, nthread );
		exit(1);
	}

	const unsigned int chunksize = (njob-1)/(num_of_processes*GRANULARITY)==0?1:(njob-1)/(num_of_processes*GRANULARITY);
	const unsigned int num_of_chunks = (njob-1)%chunksize==0?(njob-1)/chunksize:(njob-1)/chunksize+1;

	int chunk_counter_pt[1];
	pthread_mutex_t mutex_count;
	*chunk_counter_pt = num_of_chunks-1;
	calcthreads_dp1_arg_t args = { njob, chunksize, my_rank, bseq, selfscore, alloclen, neighbors, mindists, chunk_counter_pt, &mutex_count };

	if(my_rank==0) {
		pthread_t th;
		fprintf( stderr, "my_rank: %d, chunksize: %u, num_of_chunks: %u\n", my_rank, chunksize, num_of_chunks);
		pthread_mutex_init( &mutex_count, NULL );
//		calcthreads_dp1( (void *)&args );
		pthread_create(&th, NULL, calcthreads_dp1, (void *)&args ); // sub thread.  Does not use MPI when myrank==0

		calcnearest_para_thread_rank0_mpi_arg_t arg = {chunksize, neighbors, mindists, chunk_counter_pt, &mutex_count};
//		pthread_create(&th, NULL, calcnearest_para_thread_rank0_mpi, (void *)&arg );
		calcnearest_para_thread_rank0_mpi( (void *)&arg ); // main thread.  Uses MPI

		pthread_join( th, NULL );
		pthread_mutex_destroy( &mutex_count );

	} else {
		calcthreads_dp1( (void *)&args ); // main thread.  Uses MPI
	}

#if REPORTCOSTS
	reporterr( "\n" );
	reporterr( "my_rank: %d, dp1, real = %f min\n", my_rank, (float)(time(NULL) - starttime)/60.0 );
	reporterr( "my_rank: %d, dp1, user = %f min\n", my_rank, (float)(clock()-startclock)/CLOCKS_PER_SEC/60);
	reporterr( "my_rank: %d, dp1, ratio = %f", my_rank, (float)(clock()-startclock)/CLOCKS_PER_SEC / (time(NULL) - starttime) );
	use_getrusage();
	reporterr( "\n" );
#endif
} 
...
#if N0LOOPFIRST
		mem0[0] = -1;
		n0 = topolordery( mem0, topol, dep, n, 1 ) - mem0;
		mem1[0] = -1;
		n1 = topolordery( mem1, topol, dep, n, 0 ) - mem1;
#else
		mem0[0] = -1;
		n0 = topolordery( mem0, topol, dep, n, 0 ) - mem0;
		mem1[0] = -1;
		n1 = topolordery( mem1, topol, dep, n, 1 ) - mem1;
#endif
//		reporterr( "n0=%d, n1=%d\n", n0, n1 );
...
//	half = (unsigned long long)njob*(njob-1)*0.5 * LHDIVIDE;
	done = 0.0;
//	for( n=njob-2; n>-1; n-- )
	for( k=0; k<njob-1; k++ )
	{
...
#if 0
		if( done > half )
//		if( n0 * n1 < sizeav*LHBLOCKFACTOR )
		{
			blocksize0 = n0; // bunkatsu shinai
		}
		else
#endif
		{
...
typedef struct _recalcpairs_para4_thread_rank0_mpi_arg
{
//	const int numjob;
//	const unsigned int chunksize;
	const unsigned int num_of_chunks;
	int *chunk_counter_pt;
	pthread_mutex_t *mutex_count;
} recalcpairs_para4_thread_rank0_mpi_arg_t;

static void *recalcpairs_para4_thread_rank0_mpi(void *arg){
	recalcpairs_para4_thread_rank0_mpi_arg_t *targ = (recalcpairs_para4_thread_rank0_mpi_arg_t *)arg;
//	const int numjob = targ->numjob;
//	const unsigned int chunksize = targ->chunksize;
	const unsigned int num_of_chunks = targ->num_of_chunks;
	int *chunk_counter_pt = targ->chunk_counter_pt;
	pthread_mutex_t *mutex_count = targ->mutex_count;
...
//	}
	...
	for(i=first_child;i<num_of_processes;i++){
		if(*chunk_counter_pt>=num_of_chunks) break;
		MPI_Send(chunk_counter_pt,1,MPI_UNSIGNED,i,TODO,MPI_COMM_WORLD);
//		chunks_assignment[*chunk_counter_pt]=i;
		fprintf( stderr, "my_rank: %d, chunks_assignment: %d, %d\n", my_rank, *chunk_counter_pt, i);
		(*chunk_counter_pt)++;
		mpi_chunk_balance++;
	...
	while(mpi_chunk_balance){
		...
		MPI_Recv(&chunk_index,1,MPI_UNSIGNED,MPI_ANY_SOURCE,DONE,MPI_COMM_WORLD,&status);
		int sender_id = status.MPI_SOURCE;
		mpi_chunk_balance--;
		...
		if(chunk_counter<num_of_chunks){
			mpi_chunk_balance++;
			MPI_Send(&chunk_counter,1,MPI_UNSIGNED,sender_id,TODO,MPI_COMM_WORLD);
//			chunks_assignment[chunk_counter]=sender_id;
			fprintf( stderr, "my_rank: %d, chunks_assignment: %d, %d\n", my_rank, chunk_counter, sender_id);
		}
	}

	for(i=first_child;i<num_of_processes;i++){
		MPI_Send(chunk_counter_pt,1,MPI_UNSIGNED,i,EXIT,MPI_COMM_WORLD);
	}
...
typedef struct _calcthreads_dp4_arg
{
	const int njob;
	const int numjob;
	const unsigned int chunksize;
	...
	int *chunk_counter_pt;
	pthread_mutex_t *mutex_count;
} calcthreads_dp4_arg_t;

static void *calcthreads_dp4(void *arg)
{
	calcthreads_dp4_arg_t *sarg = (calcthreads_dp4_arg_t *)arg;
	const int my_rank = sarg->my_rank;
	...
	if(num_thread>0){
		const int njob = sarg->njob;
		const int numjob = sarg->numjob;
		const unsigned int chunksize = sarg->chunksize;
		const unsigned int num_of_chunks = sarg->num_of_chunks;
		char **bseq = sarg->bseq;
#if EXACTLYSAMEASPAIRLOCALALIGN
		char **bseq = sarg->dseq;
#endif
		double *selfscore = sarg->selfscore;
		int alloclen = sarg->alloclen;
		int *joborder = sarg->joborder;
		int *uselh = sarg->uselh;
		int ***topol = sarg->topol;
		Treedep *dep = sarg->dep;
		Jobplan *jobplan = sarg->jobplan;
		int *chunk_counter_pt = sarg->chunk_counter_pt;
		pthread_mutex_t *mutex_count = sarg->mutex_count;
		enum {TODO,DONE,EXIT};
		MPI_Status status;
		...
		recalcpairs4thread_arg_t_mpi *targ;
		pthread_t *handle;
		pthread_mutex_t mutex;
		...
		while(1){
			if( my_rank == 0 ) {
				...
				chunk_index = (*chunk_counter_pt)++;
				pthread_mutex_unlock( mutex_count );
				if(chunk_index>=num_of_chunks) break;
			...
			int start = chunk_index*chunksize;
			int end = (chunk_index+1)*chunksize>numjob?numjob:(chunk_index+1)*chunksize;
		...
		//	possharedn = numjob-1;
			possharedn = start;
			doneull = 0;
			targ = calloc( num_thread, sizeof( recalcpairs4thread_arg_t_mpi ) );
			handle = calloc( num_thread, sizeof( pthread_t ) );
			pthread_mutex_init( &mutex, NULL );
...
//				targ[i].numjob = numjob;
				...
				targ[i].start = start;
				targ[i].end = end;
				targ[i].my_rank = my_rank;
				pthread_create( handle+i, NULL, recalcpairs4thread_mpi, (void *)(targ+i) );
			}
		...
		//	pthread_mutex_destroy( &mutex1 );
			...
			{
//				pthread_mutex_lock( mutex_count );
//				if(*chunk_counter_pt<num_of_chunks){
//					chunk_index = (*chunk_counter_pt)++;
//				}else{
//					rank0status = EXIT; // ???
//				}
...
//				if(rank0status!=EXIT) fprintf( stderr, "my_rank: %d, chunks_assignment: %d, 0\n", my_rank, chunk_index);
			...
			{
				MPI_Send(&chunk_index,1,MPI_UNSIGNED,0,DONE,MPI_COMM_WORLD);
//				MPI_Recv(&chunk_index,1,MPI_UNSIGNED,0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
			}
		}
	}
...
#if EXACTLYSAMEASPAIRLOCALALIGN
static void recalcpairs_para4_mpi( int njob, int ***topol, Treedep *dep, char **bseq, char **dseq, double *selfscore, int alloclen, int *uselh, int *nfilesfornode )
#else
static void recalcpairs_para4_mpi( int njob, int ***topol, Treedep *dep, char **bseq, double *selfscore, int alloclen, int *uselh, int *nfilesfornode )
#endif
{
	int i;
	int numjob;
...
//	int donen;
	...
	npairs = calloc( sizeof( Pairnum ), njob );
	npairspt = calloc( sizeof( Pairnum * ), njob );
	norder = calloc( sizeof( int ), njob );
	sizeav = 0.0;
...
#else
//#if N0LOOPFIRST
//		reporterr( "N0LOOPFIRST is easy to support, but not yet supported in recalcpairs_para4_mpi.\n" );
//		exit( 1 );
//#else
		if( dep[n].child0 == -1 ) 
		{
...
// memhist wo free shinai version -> 7.383
	...
	if( my_rank == 0 )
	{
//		qsort( npairspt, njob-1, sizeof( Pairnum * ), compfuncpairpt );
//		for( i=0; i<njob-1; i++ ) norder[i] = npairspt[i]->num;
		for( i=0; i<njob-1; i++ ) norder[njob-1-i] = npairspt[i]->num;
	}

	MPI_Bcast( norder, njob-1, MPI_INT, 0, MPI_COMM_WORLD );
...
//	for( i=0; i<njob-1; i++ ) reporterr( "mergeoralign[%d] = %c\n", i, mergeoralign[i] );
	...
	reporterr( "my_rank: %d, sizeav = %f, BLOCKSIZE = %d, npairs=%lld-%lld\n", my_rank, sizeav, (int)(sizeav * LHBLOCKFACTOR), npairs[norder[0]].npairs, npairs[norder[njob-2]].npairs );
//	reporterr( "sizeav = %f, BLOCKSIZE = %d, npairs=%lld-%lld\n", sizeav, LHBLOCKSIZE, npairs[0].npairs, npairs[njob-2].npairs );
...
//			reporterr( "node %d will be divided, because n0, %d > blocksize0, %d.  nnodesdivided=%d, numjob=%d\n", n, n0, blocksize0, nnodesdivided, numjob );
	...
	reporterr( "my_rank: %d, numjob=%d, nnodesdivided=%d, njob=%d\n", my_rank, numjob, nnodesdivided, njob );
	reporterr( "my_rank: %d, Divided %d nodes to %d jobs\n", my_rank, nnodesdivided, numjob-(njob-1) );
	...
	// synchronize joborder because joborder is assigned using rand()
//	MPI_Bcast(joborder, njob, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(joborder, numjob, MPI_INT, 0, MPI_COMM_WORLD); // njob->numjob, 2016/Mar/3
...
/* all nodes */
	...
	const unsigned int chunksize = numjob/(num_of_processes*GRANULARITY)==0?1:numjob/(num_of_processes*GRANULARITY);

	const unsigned int num_of_chunks = numjob%chunksize==0?numjob/chunksize:numjob/chunksize+1;

	int chunk_counter_pt[1];
	pthread_mutex_t mutex_count;
	*chunk_counter_pt = 0;
	pthread_t th;
	calcthreads_dp4_arg_t args = {
		njob,
		numjob,
		chunksize,
		num_of_chunks,
		...
		dep,
		jobplan,
		chunk_counter_pt, // rank==0 no toki nomi shiyou
		&mutex_count      // rank==0 no toki nomi shiyou
	};

	if(my_rank==0) {
		fprintf( stderr, "my_rank: %d, chunksize: %d, num_of_chunks: %d\n", my_rank, chunksize, num_of_chunks);
		pthread_mutex_init( &mutex_count, NULL );

//		calcthreads_dp4((void *)&args);
		pthread_create(&th, NULL, calcthreads_dp4, (void *)&args ); // sub thread.  Does not use MPI when myrank==0

		recalcpairs_para4_thread_rank0_mpi_arg_t arg = {num_of_chunks, chunk_counter_pt, &mutex_count};
//		pthread_create(&th, NULL, recalcpairs_para4_thread_rank0_mpi, (void *)&arg );
		recalcpairs_para4_thread_rank0_mpi( (void *)&arg ); // main thread

		pthread_join( th, NULL );
		pthread_mutex_destroy( &mutex_count );

	} else {
		calcthreads_dp4((void *)&args); // main thread
	}
...
#if REPORTCOSTS
	reporterr( "\n" );
	reporterr( "my_rank: %d, dp4, real = %f min\n", my_rank, (float)(time(NULL) - starttime)/60.0 );
	reporterr( "my_rank: %d, dp4, user = %f min\n", my_rank, (float)(clock()-startclock)/CLOCKS_PER_SEC/60);
	reporterr( "my_rank: %d, dp4, ratio = %f", my_rank, (float)(clock()-startclock)/CLOCKS_PER_SEC / (time(NULL) - starttime) );
	use_getrusage();
	reporterr( "\n" );
#endif
}
...
//				reporterr( "checking %p->%p (height=%f)\n", b, p, p->height );
		...
		reformattree( root, treept, njob, topol, len, dep, name, treeout, 0 ); // 2017/Mar/6
...
//	int *commonanc;
	...
	if( !treegiven )
	{
		...
		calcnearest_para_mpi( njob, selfscore, dseq, alloclen, neighbors, mindists );
		MPI_Bcast(neighbors, njob, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(mindists, njob, MPI_DOUBLE, 0, MPI_COMM_WORLD);

		#if ENABLEMPIDEBUG
		if(my_rank==0){
			for(i=0;i<njob;i++) reporterr("%d, ", neighbors[i]);
			reporterr("\n");
			for(i=0;i<njob;i++) reporterr("%lf, ", mindists[i]);
			reporterr("\n");
		}		
		#endif
...
//				reporterr( "checking %p->%p (height=%f)\n", b, p, p->height );
		...
		reformattree( root, treept, njob, topol, len, dep, name, treeout, my_rank ); // 2017/Mar/6
...
#if EXACTLYSAMEASPAIRLOCALALIGN
	recalcpairs_para4_mpi( njob, topol, dep, bseq, dseq, selfscore, alloclen, uselh, nfilesfornode );
	// recalcpairs_para4( njob, topol, dep, bseq, dseq, selfscore, alloclen, uselh, nfilesfornode );
#else
	recalcpairs_para4_mpi( njob, topol, dep, bseq, selfscore, alloclen, uselh, nfilesfornode );
...
//	recalcpairs( njob, topol, dep, bseq, selfscore, alloclen, hat3node, fd0, fd1, uselh );
	...
	#if ENABLEMPIDEBUG
	reporterr("my_rank: %d, end compacttreedpdist\n", my_rank);
	#endif
//	free( commonanc );
//	commonsextet_p( NULL, NULL );
